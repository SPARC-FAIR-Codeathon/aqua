...
import os
gmail_user = os.getenv('NM_EMAIL_USR', None)
gmail_password = os.getenv('NM_EMAIL_PWD', None)
dbname = os.getenv('DB_NAME', None)
api_key = os.getenv('ES_API_KEY', None)
es_portal = 'https://scicrunch.org/api/1/elastic/SPARC_PortalDatasets_pr/_search'
...
def create_notifyme_db(db_name, clean=False):
    ...
    cur.execute('CREATE TABLE IF NOT EXISTS \
                new_register (entry_id INTEGER PRIMARY KEY AUTOINCREMENT, \
                email VARCHAR, register_date VARCHAR ,keywords VARCHAR, \
                status VARCHAR, last_modified VARCHAR )')
    cur.execute('CREATE TABLE IF NOT EXISTS waiting_list (entry_id INTEGER, \
                email VARCHAR, register_date VARCHAR ,keywords VARCHAR, \
                status VARCHAR, last_modified VARCHAR, hits INTEGER, \
                failed_reqid INTEGER )')
    cur.execute('CREATE TABLE IF NOT EXISTS archieved_list (entry_id INTEGER, \
                email VARCHAR, register_date VARCHAR ,keywords VARCHAR, \
                status VARCHAR, last_modified VARCHAR,hits INTEGER, \
                failed_reqid INTEGER )')
    cur.execute('CREATE TABLE IF NOT EXISTS Failed_emails \
                (failed_reqid INTEGER PRIMARY KEY AUTOINCREMENT, \
                corresponding_entry_id INTEGER, error_message VARCHAR, \
                error_date VARCHAR )')
    conn.commit()
    conn.close()
...
# The main function to be called by the front end to save the entries
def add_new_single_entry(email,keywords):
    conn = sqlite3.connect(dbname)
    cur = conn.cursor()
    cur.execute('INSERT INTO new_register (email, register_date,keywords , \
                status , last_modified ) VALUES (?, CURRENT_TIMESTAMP, ? , \
                "New" , CURRENT_TIMESTAMP)', (email,keywords ))
    conn.commit()
    conn.close()

# to get how many hits currently exist against keywords,
# mainly used in scan_new_register
def get_hits_count(keyword):
    rsp = requests.get(es_portal + '?api_key=' + api_key + '&q=' + keyword)
    rsp = json.loads(rsp.text)
    return rsp['hits']['total']

# to check if there is any duplicate entries and archieve those for efficency,
# mainly called in scan_new_register and scan_waiting_list()
def archieve_duplicates(df):
    ...
    return df
# This is the main function to retrieve search results as a pandas df to embedd
# in the email message
def retrieve_search_to_df(keyword):
    rsp = requests.get(es_portal + '?api_key=' + api_key + '&q=' + keyword)
    ### need to make sure I got the most matching and get the top 10,
    # that should use our improved search and not just the k core
    rsp = json.loads(rsp.text)
    if rsp['hits']['total']==0:
...
# This function is to send email and capture errors, normally called for records
# with hits>0 in scan_waiting_list
...
def update_failed_email(entry_id,exception):
    conn = sqlite3.connect(dbname)
    cur = conn.cursor()
    cur.execute('INSERT INTO Failed_emails(corresponding_entry_id,error_message, \
                error_date) VALUES (?,?,  CURRENT_TIMESTAMP)',(entry_id,exception))
    conn.commit()
    cur.execute('Update waiting_list set status="Failed",failed_reqid=(select \
                max(failed_reqid) from Failed_emails where corresponding_entry_id=? ) \
                where entry_id=?', (entry_id,entry_id ))
    conn.commit()
    conn.close()
...
def scan_new_register():
    ...
    cur = conn.cursor()
    cur.execute('Delete from new_register where entry_id in \
                (select distinct entry_id from waiting_list)' )
    cur.execute('Delete from new_register where entry_id in \
                (select distinct entry_id from archieved_list)' )
    conn.commit()
    conn.close()

# this is the next function following the scan of new entries, and the
# resposible one for sending emails and capture errors
def scan_waiting_list():
    ...
    conn = sqlite3.connect(dbname)
    cur = conn.cursor()
    cur.execute("Delete from waiting_list where entry_id in \
                (select distinct entry_id from archieved_list)" )
    df['updated_hits']=0
    ...
    conn.commit()
    conn.close()
    ### Store the failed entries in the failed table and flag the failed
    ### entries in the waiting list table
